...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QSpinBox, QCalendarWidget,
)
from PyQt6.QtCore import Qt, QThread, pyqtSignal, pyqtSlot, QUrl, QDate
...
def canonical_exercise(name: str) -> str:
    ...
    aliases = {
        "squats": "Squats",
        "mini squats": "Squats",                 # you can change if you want separate
        "mini-squats": "Squats",
...
class SetupPage(QWidget):
    ...
    def __init__(self):
        ...
        # Recording label (used for tagging recorded videos)
        lbl_row = QHBoxLayout()
        ...
        # Doctor exercise selection (controls thresholds)
        ex_row = QHBoxLayout()
        ...
        # ── Bottom controls (scroll-capped)
        ctrl_widget = QWidget()
        ctrl_layout = QVBoxLayout(ctrl_widget)
        ctrl_layout.setContentsMargins(0, 0, 0, 0)
        ctrl_layout.setSpacing(6)

        # ── Per-exercise thresholds (spinboxes)
        ...
        # ── Wiring (MUST be after widgets exist)
    ...
    # ── Per-exercise thresholds (load/save)
    ...
    def _on_exercise_changed(self, exercise_name: str):
        ...
        if not isinstance(vals, dict):
            # Backward-compat: if old global thresholds exist, use them once as defaults
    ...
    def _persist_exercise_setup(self):
        ...
        # ✅ Canonical key (this is the important line)
            ...
            try:
                ...
                for i, v in enumerate(videos_list):
                    ...
                    if vpath and Path(vpath).exists():
                        ...
                        # Mark as model ONLY if this video's path matches the model for its exercise
                        model_path_for_ex = ""
                        mv = model_videos.get(ex_key)
    ...
    # ── Setup saving/loading (angle targets)
    def _save_setup_full(self):
        ...
        # ensure current exercise thresholds are saved too
        ex_map = setup.get("exercise_thresholds")
        if not isinstance(ex_map, dict):
            ex_map = {}
        ...
        data["setup"] = setup
        data["videos"] = videos_payload  # backward compat
    ...
    def _on_model_selected(self, slot: "VideoSlotWidget"):
        # Set model only for THIS slot's exercise (not global)
    ...
    def _save_model_video(self, slot: "VideoSlotWidget"):
        ...
        # Backward compatibility: keep also the old single model_video (last selected)
...
class ReportsPage(QWidget):
    ...
    def __init__(self):
        ...
        # Date picker — opens a full calendar dialog
    ...
    def _open_date_picker(self):
        """Open the calendar date picker and update the button label."""
...
class PatientDashboard(QDialog):
    ...
    def __init__(self, parent=None, standalone: bool = False):
        ...
        sidebar.setMaximumWidth(260)   # optional, keeps it from growing too wide
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        root.addWidget(split, stretch=1)
                ...
                # ── Stats bar (create the labels used in select_exercise/update_stats) ──
        stats = QFrame()
        ...
        # Optional: side stats for Straight Leg Raises
        ...
        # Hide side columns by default (only show for Straight Leg Raises)
        ...
        admin_btn_label = "Patient Dashboard" if CURRENT_USER_ROLE == "admin" else "My Profile"
    ...
    def _on_media_status(self, status):
        # When video ends, restart it if looping is enabled and the session is running
